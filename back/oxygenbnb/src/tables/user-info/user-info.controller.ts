import { Controller, Get, Body, Patch, Param, ParseIntPipe, HttpException, HttpStatus } from '@nestjs/common';
...
export class UserInfoController {
  ...
  @Get(':user_id')
  async findOneByUserId(@Param('user_id', ParseIntPipe) user_id: number): Promise<UserInfo> {
    const userInfo = await this.userInfoService.findOneByUserId(user_id);
    if (userInfo) return userInfo;
    else throw new HttpException(`User info not found for user ${user_id}`, HttpStatus.NOT_FOUND);
  }
