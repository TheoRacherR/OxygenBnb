...
describe('UserService', () => {
  ...
  describe('findOneById', () => {
    ...
    it('should throw a not found exception', () => {
      ...
      expect(() => service.findOneById(id)).rejects.toBeInstanceOf(
        NotFoundException,
      );
  ...
  describe('update', () => {
    it('should return { message: `User 1 updated` }', () => {
      const id = 1;
      jest.spyOn(UserDbMock.user, 'findOneById').mockResolvedValue(userMock[0]);
      expect(service.update(id, userMock[0])).resolves.toEqual({
    ...
    it('should throw a not found exception', () => {
      const id = 1;
      jest.spyOn(UserDbMock.user, 'findOneById').mockResolvedValue(undefined);
      expect(() => service.update(id, userMock[0])).rejects.toBeInstanceOf(
        NotFoundException,
      );
      expect(() => service.update(id, userMock[0])).rejects.toEqual(
        new NotFoundException('Not Found'),
      );
    });
  });
  ...
  describe('delete', () => {
    it('should return { message: `User 1 deleted` }', () => {
      const id = 1;
      jest.spyOn(UserDbMock.user, 'findOneById').mockResolvedValue(userMock[0]);
      expect(service.delete(id)).resolves.toEqual({
    ...
    it('should throw a not found exception', () => {
      const id = 1;
      jest.spyOn(UserDbMock.user, 'findOneById').mockResolvedValue(undefined);
      expect(() => service.delete(id)).rejects.toBeInstanceOf(
        NotFoundException,
      );
      expect(() => service.delete(id)).rejects.toEqual(
        new NotFoundException('Not Found'),
      );
    });
  });
