...
export class RentalDateController {
  ...
  async findOneById(
    ...
    const rentalDate = await this.rentalDateService.findOneById(id);
    if (rentalDate) return rentalDate;
    else
      throw new HttpException(
        `RentalDate ${id} not found`,
  ...
  @Get('type/:type')
  async findAllByType(
    @Param('type') type: rentalDateType,
  ): Promise<RentalDate[]> {
    const rentalDateFoundByType =
      await this.rentalDateService.findAllByType(type);
    if (rentalDateFoundByType) return rentalDateFoundByType;
    else
      throw new HttpException(
        `No Rental date for type ${type} where found`,
        HttpStatus.NOT_FOUND,
      );
  }

  @Get('rental/:rental_id')
  async findAllByRentalId(
    @Param('rental_id') rental_id: number,
  ): Promise<RentalDate[]> {
    const rentalDateFoundByRentalId =
      await this.rentalDateService.findAllByRentalId(rental_id);
    if (rentalDateFoundByRentalId) return rentalDateFoundByRentalId;
    else
      throw new HttpException(
        `No Rental date for rental_id ${rental_id} where found`,
        HttpStatus.NOT_FOUND,
      );
  }
  ...
  @Post()
  async create(
    @Body() rentalDate: CreateRentalDateDto,
  ...
  async update(
    @Param('id', ParseIntPipe) id: number,
    @Body() rentalDate: UpdateRentalDateDto,
  ): Promise<{ message: string }> {
    const rentalDateFoundById = await this.rentalDateService.findOneById(id);
    if (!rentalDateFoundById)
      throw new HttpException(
        `No Rental date ${id} where found`,
        HttpStatus.NOT_FOUND,
      );
