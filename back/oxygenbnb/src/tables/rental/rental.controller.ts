...
export class RentalController {
  ...
  @Get('owner/:user_id')
  async findOneByUserId(@Param('user_id', ParseIntPipe) user_id: number): Promise<Rental[]> {
  ...
  @Get('type/:type')
  async findOneByType(@Param('type') type: rentalType): Promise<Rental[]> {
  ...
  @Post()
  async create(@Body() rental: CreateRentalDto): Promise<Rental> {
    return this.rentalService.create(rental);
  }

  @Patch(':id')
  async pdate(@Param('id', ParseIntPipe) id: number, @Body() rental: UpdateRentalDto): Promise<Rental> {
