...
const SearchLocalisation = ({error}) => {
  ...
  const [query, setQuery] = useState(JSON.parse(locationInformations.localisation_infos)?.display_name || '');

  const handleSelectCity = (item) => {
    setLocationInformations(prev => ({...prev, localisation_infos: JSON.stringify(item)}));
  ...
  const searchForQuery = () => {
    if (query.length > 0) {
      ...
      fetch(`${NOMINATIM_BASE_URL}${queryString}`, {
        ...
        .then((result) => {
          {error}setListOfLocalisations(JSON.parse(result).filter((r) => r.addresstype === "place" || r.addresstype === "building"));
    ...
    <>
      <div style={{ padding: " 5px" }}>
        <Input
          ...
          endDecorator={
            <Button onClick={searchForQuery} color="neutral"><SearchRoundedIcon/></Button>
          }
        />
        {searchedBoolean ? (
          listOfLocalisations.length === 0 ? (
            ...
            listOfLocalisations.map((item, index) => (
              <MenuItem key={index} onClick={() => handleSelectCity(item)} sx={{overflowX: "hidden"}}>
                {/* {item.address.city ||
                  item.address.town ||
